...
\begin{solution}
  \begin{itemize}
  \item[a.] \(\exists a\forall b (\psi(b)\rightarrow\phi(a, b))\);
  \item[b.] \(\exists a, b, c (\psi(a, b) \oplus \psi(b, c))\);
  \item[c.] \(\forall a, b, c (\psi(a, c) \land \psi(b, c) \rightarrow \perp)\).
  \end{itemize}
\end{solution}
...
\begin{solution}
  Let \(\phi\) be the formula \(\forall a\forall b (arb \rightarrow\forall c (b\not r c))\).
